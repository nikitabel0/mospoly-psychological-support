...
1. **Model** - создать ORM модель
2. **Repository** - CRUD операции
3. **Service** - бизнес-логика
4. **Schema** - Pydantic модели
5. **Controller** - API endpoints
6. **Router** - подключить в main
...
### Добавление dependency

```python
async def get_current_user(
    token: str = Cookie(alias="access_token")
) -> User:
